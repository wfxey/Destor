...
#Current Version
current_version = "v24.6.28-official"
...
#Delete Server
def delete_server():
    selected_option = radio_var.get()  # Stellen Sie sicher, dass radio_var definiert ist
...
class Config_Server:
    ...
    def show_progress_window(self, jar_url, server_dir):
        ...
        label = ctk.CTkLabel(root, text="By downloading, you accept Minecraft's EULA.", font=("Open Sans", 12))
...
def start_server():
    ...
    try:
        ...
        threading.Thread(target=read_output, args=(server_process.stdout, 'info')).start()
        threading.Thread(target=read_output, args=(server_process.stderr, 'error')).start()
...
def handle_keypress(event):
    ...
    if console_text:
        if event.keysym not in ['BackSpace', 'Delete']:
            ...
            console_text.tag_config('input', foreground='green')  # Custom tag for input text
